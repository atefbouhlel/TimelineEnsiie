...
class UserController extends BaseController {
	...
	{
		$view=View::make('layout.showProfile');
		return $view;
	}
	...
	public function signUp()
	{
		...
		if(User::checkEmail($email)==false){
			...
			if(Input::hasFile('photo')){
				...
				$name=str_random(10).Input::file('photo')->getClientOriginalName();
				$photo=Input::file('photo')->move($dest,$name);
			}
			else
	...
	public function search()
	{
		...
		foreach($users as $user){
			if(  (Str::contains(Str::lower($user->prenom),Str::lower($keywords))) or (Str::contains(Str::lower($avocat->nom),Str::lower($keywords)))   )	
				$searchUsers->add($user);
		}

		return View::make('layout.searchedUsers')->with('searchUsers',$searchUsers);
